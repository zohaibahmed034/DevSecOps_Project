    ...
    stages {
        stage('Step 1: Checkout & Setup') {
            steps {
                checkout scmGit(
                    branches: [[name: 'main']], 
                    userRemoteConfigs: [[url: "https://github.com/${REPO_NAME}.git", credentialsId: 'github-token']]
                )
                container('scanner') {
...
EOF
        ...
        stage('Final: Archive Audit Reports to S3') {
        steps {
            script {
                // Use a lightweight AWS CLI container
                container('aws-cli') {
                    withVault([ [path: 'secret/devsecops/creds', variables: [
                        [name: 'AWS_AK', column: 'aws_access_key'],
                        [name: 'AWS_SK', column: 'aws_secret_key']
                    ]] ]) {
                        sh """
                            ...
                            echo "--- 📦 Archiving Build #${env.BUILD_NUMBER} Reports ---"
                            
                            # Source directory where your scanners saved their files
                            REPORT_SRC="/var/reports-dir/build-${env.BUILD_NUMBER}"
                            
                            # Destination Bucket
                            BUCKET_NAME="devsecops-audit-reports-adeel"
                            
                            if [ -d "\$REPORT_SRC" ]; then
                                echo "--- 📤 Uploading to s3://\$BUCKET_NAME/build-${env.BUILD_NUMBER}/ ---"
                                
                                # Sync ensures only the files in the build folder are uploaded
                                aws s3 sync \$REPORT_SRC/ s3://\$BUCKET_NAME/build-${env.BUILD_NUMBER}/ --quiet
                                
                                echo "--- ✅ Successfully archived to S3 ---"
                            else
                                echo "--- ❌ Error: No report directory found at \$REPORT_SRC ---"
                                exit 1
                            fi
                        """
                    }
