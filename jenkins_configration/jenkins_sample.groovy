...
"""
    ...
    environment {
        ...
        ARGO_SERVER = '100.24.125.225:31436'
    ...
    stages {
        ...
        stages {
        stage('Step 6.1:Initialize CLI') {
            steps {
                sh '''
                    if [ ! -f "${ARGOCD_BIN}" ]; then
        ...
        stage('Step 6.2:Smart Deploy') {
            steps {
                script {
                    withCredentials([string(credentialsId: 'argocd-token', variable: 'ARGO_TOKEN')]) {
                        
                        // LOGIC: Check if app exists
                        // returnStatus: true pipeline ko crash nahi hone deta agar app na mile
                        def appExists = sh(
                            script: "${ARGOCD_BIN} app get ${APP_NAME} --server ${ARGO_SERVER} --auth-token '${ARGO_TOKEN}' --insecure",
                            returnStatus: true
                        )

                        if (appExists != 0) {
                            echo "--- LOGIC: Application not found. Creating fresh... ---"
                            sh """
                                ${ARGOCD_BIN} app create ${APP_NAME} \
                                    --server ${ARGO_SERVER} \
                            ...
                            echo "--- LOGIC: Application already exists. Syncing updates... ---"
                            // Agar app pehle se hai, to sirf refresh aur sync karein
                            sh "${ARGOCD_BIN} app sync ${APP_NAME} --server ${ARGO_SERVER} --auth-token '${ARGO_TOKEN}' --insecure"
                        }
                    }
                }
            }
        ...
        stage('Step 6.3:Final Validation') {
            steps {
                script {
                    withCredentials([string(credentialsId: 'argocd-token', variable: 'ARGO_TOKEN')]) {
                        echo "Waiting for Sync to complete..."
                        sh "${ARGOCD_BIN} app wait ${APP_NAME} --server ${ARGO_SERVER} --auth-token '${ARGO_TOKEN}' --sync --health --timeout 300 --insecure"
