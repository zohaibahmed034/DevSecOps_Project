...
spec:
  ...
  - name: owasp-check
    ...
    - name: owasp-reports
      mountPath: /var/owasp-dependency-check
  - name: checkov
  ...
  - name: python-risk
    ...
    - name: devsecops-reports
      mountPath: /var/devsecops-reports
    - name: owasp-reports
      mountPath: /var/owasp-dependency-check
  - name: docker-daemon
  ...
  volumes:
  - name: report-storage
    hostPath: { path: /var/reports-dir, type: DirectoryOrCreate }
  - name: devsecops-reports
    hostPath: { path: /var/devsecops-reports, type: DirectoryOrCreate }
  - name: owasp-reports
    hostPath: { path: /var/owasp-dependency-check, type: DirectoryOrCreate }
  - name: odc-data
    hostPath: { path: /var/owasp-dependency-check/data, type: DirectoryOrCreate }
"""
        }
    }
    ...
    environment {
        ...
        DEVSECOPS_DIR    = "/var/devsecops-reports"
        OWASP_DIR        = "/var/owasp-dependency-check"
    }
    ...
    stages {
        ...
        stage('Step 6: Risk Scoring Gate') {
            steps {
                container('python-risk') {
                    script {
                        sh """
                            echo "--- Centralizing Reports ---"
                            mkdir -p /tmp/all-reports
        
                            # In variables ka istemal karein jo env mein define hain
                            cp -v ${env.REPORT_DIR}/* /tmp/all-reports/ 2>/dev/null || true
                            cp -v ${env.DEVSECOPS_DIR}/* /tmp/all-reports/ 2>/dev/null || true
                            cp -v ${env.OWASP_DIR}/* /tmp/all-reports/ 2>/dev/null || true
        
                            # Symlink for risk_engine.py
                            rm -rf /reports-dir
                            ln -s /tmp/all-reports /reports-dir
