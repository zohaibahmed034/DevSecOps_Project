...
"""
    ...
    stages {
        ...
        stage('Step 6: Unified Release to GitHub') {
            steps {
                container('tools') {
                    withCredentials([string(credentialsId: 'github-token', variable: 'GITHUB_TOKEN')]) {
                        script {
                            def releaseTag = "DevSecOps-Full-Build-${env.BUILD_NUMBER}"
                            sh """
                                apk add --no-cache curl zip
                                cd ${REPORT_DIR}
                                zip -r combined-security-report.zip ./*
                                
                                # Create GitHub Release
                                curl -X POST -H "Authorization: token ${GITHUB_TOKEN}" \
                                     -H "Accept: application/vnd.github.v3+json" \
                                     https://api.github.com/repos/${REPO_NAME}/releases \
                                     -d '{"tag_name":"${releaseTag}","name":"Release ${releaseTag}","body":"Combined Security Scan Results for Build ${env.BUILD_NUMBER}","draft":false,"prerelease":false}' > release_resp.json
                                ...
                                curl -X POST -H "Authorization: token ${GITHUB_TOKEN}" \
                                     ...
                                     --data-binary @"combined-security-report.zip" \
                                     "https://uploads.github.com/repos/${REPO_NAME}/releases/\$RELEASE_ID/assets?name=combined-security-report-build-${env.BUILD_NUMBER}.zip"
                            """
                        }
                    }
                }
